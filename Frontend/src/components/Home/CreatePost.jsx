...
//   // ✅ Access the name from the Redux store safely
//   const user = useSelector((state) => state.user) || {}; 
//   const name = user.name || "Unknown User"; // Fallback if user is undefined
...
export function CreatePost({ onPost }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = new FormData();
    formData.append("description", content);
    formData.append("name", displayName); // Send the username or companyName from Redux
    if (media?.file) {
      formData.append("media", media.file);
    }
    ...
    >
      <form onSubmit={handleSubmit}>
        <div className="flex gap-4">
          ...
          <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0">
            <img
              src={user?.profilePic || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&h=100&fit=crop&q=80"}
              alt={displayName}
              className="w-full h-full object-cover"
            />
          </div>
